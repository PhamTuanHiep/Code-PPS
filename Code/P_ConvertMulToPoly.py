                                    ...
                                    for d in range(c+1,n):
                                        ...
                                        for e in range(d+1,n):
                                            ...
                                            for f in range(e+1,n):
                                                a11=a11+X[i]*X[j]*X[k]*X[p]*X[q]*X[a]*X[b]*X[c]*X[d]*X[e]*X[f]
                                                if(n==11):
                                                    continue
                                                for g in range(f+1,n):
                                                    a11=a11+X[i]*X[j]*X[k]*X[p]*X[q]*X[a]*X[b]*X[c]*X[d]*X[e]*X[f]*X[g]
                                                    if(n==12):
                                                        continue
    if(n==1):
        A=np.array([1,-a1])
        ...
        A=np.array([1,-a1,a2,-a3,a4,-a5,a6,-a7,a8,-a9,a10,a11])
    elif(n==12):
        A=np.array([1,-a1,a2,-a3,a4,-a5,a6,-a7,a8,-a9,a10,a11,a12])
    return A
# X=[1,2,3,4]
...
#Đạo hàm da thuc f theo data bậc cập n ------------------------------------------------------------------
def DH_dt(X,n,name):
    x = sp.symbols(name)
    f=F(X,name)
    f = sp.diff(f, x,n)
...
# n=2
# name="x"
# print("Dao ham bac %d"%(n),DH_dt(X,n,name))
